...
// SCROLL_THRESHOLD, the bar slides up under the navbar via a transform
// — giving them the full viewport for browsing. Any upward scroll
// cancels the pending hide and slides the bar back into view, so it's
// always one flick away.
//
// We use a transform (not display:none / position changes) so the
// transition is GPU-friendly and doesn't reflow the page. The element
// keeps its layout space while pinned — translateY(-100%) just moves
// it up by its own height, which puts it behind the already-sticky
// navbar (z:30 > our z:20).
    ...
    const onScroll = () => {
      ...
      if (newDirection && newDirection !== direction) {
        ...
        if (newDirection === "up") {
          ...
          // Schedule hide. Direction change to up cancels via clearHideTimer.
